...
## 🥺 느낀점
- 설계와 일정 관리의 중요성을 느낀 3주 였다. 일과 사이드 프로젝트를 병행하는 것이 쉽지 않았다. 프로젝트가 끝나갈 때쯤에 JIRA 를 사용했으면 꽤 효과적으로 진행했을 것 같은 느낌이 강하게 들었다. 앞으로 사이드 프로젝트 할 때는 JIRA를 필수로 사용해야겠다.
- Exception 처리, 예상되는 결함 처리 (숫자 범위 등..), 보안적인 부분 등 디테일한 것들을 제대로 하지 못해 아쉬웠다. 공식적인 프로젝트 기간은 끝났지만 앞으로도 개선하면서 경험치를 쌓으려고 한다.
